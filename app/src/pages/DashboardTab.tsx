  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Ingresos vs gastos por mes" className="chart">
      ...
      {series.map((s, i) => {
        ...
        return (
          <g key={s.month}>
            <rect x={x - BW - 2} y={H - PAD - hIn} width={BW} height={Math.max(hIn, 1)} rx="3" style={{ fill: 'var(--chart-green)' }} />
            <rect x={x + 2} y={H - PAD - hEx} width={BW} height={Math.max(hEx, 1)} rx="3" style={{ fill: 'var(--chart-blue)' }} />
            <text x={x} y={H - 8} textAnchor="middle" fontSize="11" style={{ fill: 'var(--chart-axis)' }}>
      ...
      <g>
        <rect x={W - 150} y={6} width="10" height="10" rx="2" style={{ fill: 'var(--chart-green)' }} />
        <text x={W - 135} y={15} fontSize="11" style={{ fill: 'var(--chart-text)' }}>Ingresos</text>
        <rect x={W - 75} y={6} width="10" height="10" rx="2" style={{ fill: 'var(--chart-blue)' }} />
        <text x={W - 60} y={15} fontSize="11" style={{ fill: 'var(--chart-text)' }}>Gastos</text>
      </g>
    </svg>
  )
}
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Energía diaria por huésped y admin" className="chart">
      ...
      {[0, 0.25, 0.5, 0.75, 1].map((f) => {
        ...
        return (
          <g key={f}>
            <line x1={PADL} x2={W - PAD} y1={y} y2={y} style={{ stroke: 'var(--chart-grid)' }} strokeWidth="1" />
            <text x={PADL - 6} y={y + 3} textAnchor="end" fontSize="9" style={{ fill: 'var(--chart-axis)' }}>
              {(niceMax * f).toFixed(1)}
            </text>
          </g>
        )
      ...
      {/* Línea de promedio */}
      <line x1={PADL} x2={W - PAD} y1={avgY} y2={avgY} style={{ stroke: 'var(--chart-green)' }} strokeWidth="1.5" strokeDasharray="5 4" />
      <text x={W - PAD - 2} y={avgY - 5} textAnchor="end" fontSize="9" style={{ fill: 'var(--chart-green)' }} fontWeight="600">
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={d.date}>
            ...
            {d.kwh_admin > 0 && <rect x={x - bw / 2} y={PADL + chartH - hA} width={bw} height={Math.max(hA, 1)} rx="3" style={{ fill: 'var(--chart-axis)' }} />}
            {d.kwh_guest > 0 && <rect x={x - bw / 2} y={PADL + chartH - hA - hG} width={bw} height={Math.max(hG, 1)} rx="3" style={{ fill: 'var(--chart-blue)' }} />}
            <text x={x} y={PADL + chartH - hA - hG - 6} textAnchor="middle" fontSize="9.5" style={{ fill: 'var(--chart-text)' }} fontWeight="600">
              {total > 0 ? total.toFixed(1) : ''}
            </text>
            <text x={x} y={H - 8} textAnchor="middle" fontSize="9.5" style={{ fill: 'var(--chart-axis)' }}>
              {d.date.slice(8)}
            </text>
          </g>
        )
      ...
      <g>
        <rect x={PADL} y={6} width="10" height="10" rx="2" style={{ fill: 'var(--chart-blue)' }} />
        <text x={PADL + 14} y={15} fontSize="10.5" style={{ fill: 'var(--chart-text)' }}>Huésped</text>
        <rect x={PADL + 78} y={6} width="10" height="10" rx="2" style={{ fill: 'var(--chart-axis)' }} />
        <text x={PADL + 92} y={15} fontSize="10.5" style={{ fill: 'var(--chart-text)' }}>Admin</text>
      </g>
    </svg>
  )
}
